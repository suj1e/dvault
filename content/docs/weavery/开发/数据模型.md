...
前后端开发都据此设计 DB 与 API。功能行为以 8 篇功能文档为准(见 [[产品/功能/]]),权限细节见 [[权限矩阵]],异常处理见 [[异常与边界]]。
...
erDiagram
    ...
    User ||--o| AiAuthorConfig : "configured-as-ai-author"
    User ||--o{ AiPublishTask : "produces-via"
...
| `accountType`         | enum         | 必填        | 账号身份:`human` / `ai_author`(AI 作者),见 [[设计/角色与身份模型]]        |
| `roles`               | json array   | 默认 `[]`   | 职责权限位:`["curator"]` / `["admin"]` 可叠加;`ai_author` 时必须为 `[]`   |
...
- `accountType`:`human`(普通用户 / 认证作者,认证作者 `verified=true`)、`ai_author`(AI 作者,如云小织,region"云端")
- `roles`:职责权限位数组,可叠加(`curator` 策展人 / `admin` 管理员);**`accountType=ai_author` 时必须为 `[]`**(AI 作者不持职责)
- 游客 = 未登录(无 User 记录);所有者 = `resource.author_id == user.id`(非字段,见 [[权限矩阵#角色]])

**`settings` 子对象**(对应设置页三组,见 [[产品/功能/账户与治理#设置]]):
...
|                   | `default_model`      | enum | `gpt-4o`  | 工作台默认模型(Q6 已决:清单含 GPT-4o / Claude 3.7 / DeepSeek-V3 / 通义千问,见 [[待确认问题#Q6]]) |
...
| **`postOrigin`**     | enum           | 必填          | 内容来源:`human` / `ai_assisted` / `ai_generated`,见 [[设计/角色与身份模型]]                                   |
| **`human_revised`**  | bool           | 默认 false    | 策展人改稿标记(仅 `postOrigin=ai_generated` 可为 true),UI 显示"经人工修订"                                     |
| **`ai_model`**       | string         | 可空          | 如 `GPT-4o` / `Claude 3.7` / `DeepSeek-V3` / `通义千问`                                                        |
...
| **`version`**        | int            | 默认 1        | **乐观锁版本号**(Q29),每次保存 +1;多端并发编辑时后提交者收 409 Conflict,见 [[异常与边界#C2. 作者多端同时编辑]] |

**状态枚举**:`draft`(草稿,仅作者可见)、`published`(已发布,进 feed)、`deleted`(作者删除,作者删除二次确认 2.5s,见 [[产品/功能/帖子详情与Remix]])。

**关系**:N:1 `User`(author)、N:1 `Topic`、1:N `Comment`、1:N `Favorite`、N:1 `Post`(Remix 出处,自引用)、1:N `AiUsage`。

> [!important] AI 字段是透明文化的数据基础
> 加粗的 AI 相关字段(`postOrigin` / `human_revised` / `ai_model` / `prompt_text` / `prompt_params` / `prompt_version`)是 weavery "AI 内容强制标注 + 提示词公开" 制度的数据底座,必须保留并随作品持久化。`postOrigin` 由"系统建议 + 创作者确认 + 抽审"判定(见 [[设计/角色与身份模型#三postorigin-判定机制]])。社区准则第 2 条明文要求,见 [[产品/功能/账户与治理#社区准则]]。

---

### 草稿 `Draft`

> 工作台的未完成稿件。自动保存于本地 `localStorage` 与服务端,统计见 [[产品/功能/创作者中心#草稿箱]]。
...
> 内容聚合标签,`#`+名。内置 19 个,见 [[产品/功能/话题与搜索#内置话题完整清单]]。
...
> [!note] 原型范围
> 评论在原型里无排序无分页,见 [[产品/功能/帖子详情与Remix#评论区]]。生产实现需补分页与排序策略(由后端定)。
...
> [!important] 隐私限权
> 发起私信受 `User.settings.privacy.allow_dm_all` 限制——关闭后仅关注者可发起,见 [[产品/功能/账户与治理#隐私]]。
...
> [!important] 仅供娱乐参考
> AI 评价必须显示免责声明 **"以上评价由社区 AI 自动生成,仅供娱乐参考 · 基于 {posts} 篇作品与 {likes} 次获赞"**,不影响实际权重。每个用户预设 **3 个 review 版本**,点击"重新生成"按 `(idx+1) % 3` 循环切换下一版,见 [[产品/功能/作者主页#AI 评价]]。
...
**dashboard 来源环形图聚合**(见 [[产品/功能/创作者中心#创作者数据]]):
...
> [!info] 配额机制(Q27 已决:M1 不设上限)
> M1 阶段 AI 创作用量**全部免费、不设上限**,优先让用户跑通"AI 辅助创作"的核心体验;`AiUsage` 仅作统计与追溯,不做配额拦截。后续版本再引入配额(额度、重置周期、是否区分模型、是否对游客关闭),届时不计费的记录仍保留,便于历史数据回溯。详见 [[异常与边界#A5. AI 用量超限]]。

### AI 作者配置 `AiAuthorConfig`

> AI 作者账号的内容生产配置,驱动**定期自动发布**。详见 [[AI作者与内容生产]]。

**字段**:

| 字段            | 类型         | 约束                | 说明                                                                                                                  |
| --------------- | ------------ | ------------------- | --------------------------------------------------------------------------------------------------------------------- |
| `id`            | string(uuid) | PK                  | 配置唯一标识                                                                                                          |
| `ai_author_id`  | string(uuid) | FK→User,必填,unique | 关联的 AI 作者账号(`User.accountType=ai_author`,如云小织)                                                             |
| `enabled`       | bool         | 必填,默认 true      | 是否启用自动生产                                                                                                      |
| `schedule_cron` | string       | 可空                | 定时调度表达式(定时定期,如"每日 09:00"=`0 9 * * *`、"每周一"=`0 10 * * 1`)                                            |
| `topic_source`  | json         | 可空                | 选题来源配置:话题池 / 热点抓取 / 运营指定                                                                             |
| `model`         | string       | 可空                | 生成所用默认模型(如 `GPT-4o`),见 [[待确认问题#Q6]]。Q6 已决:模型清单含 GPT-4o / Claude 3.7 / DeepSeek-V3 / 通义千问等 |
| `review_mode`   | enum         | 必填                | **`auto`(全自动直发)/ `manual`(人工审校)**,见下表                                                                     |
| `curator_id`    | string(uuid) | FK→User,可空        | `manual` 模式下的责任策展人                                                                                           |
| `created_at`    | datetime     | 必填                | 配置创建时间                                                                                                          |

**review_mode 枚举**(核心配置,见 [[AI作者与内容生产#四、审核模式核心]]):

| review_mode | 流程                          | 适用                           |
| ----------- | ----------------------------- | ------------------------------ |
| `auto`      | AI 生成 → 自动发布            | 高频/低风险/模板化内容         |
| `manual`    | AI 生成 → 审校队列 → 人工通过 | 重点/标杆/实验性内容(如云小织) |

**关系**:1:1 `User`(ai_author)。

> [!info] 云小织对应 manual 模式
> 云小织 profile 文案"内容由人类策展人审校后发布"对应 `review_mode=manual`,是官方标杆作者的质量兜底配置。其他 AI 作者通道可配 `auto`。两种模式并存,按配置切换。

### AI 发布任务 `AiPublishTask`

> 每次调度触发产生一条任务记录,可追溯生成 → 审核 → 发布全链路。
...
| `config_id`     | string(uuid) | FK→AiAuthorConfig,必填 | 触发此任务的配置                                                      |
| `ai_author_id`  | string(uuid) | FK→User,必填           | 执行生产的 AI 作者                                                    |
| `post_id`       | string(uuid) | FK→Post,可空           | 产出文章(生成后绑定)                                                  |
| `topic`         | string       | 可空                   | 本次选题                                                              |
| `model`         | string       | 必填                   | 实际使用的模型                                                        |
| `review_mode`   | enum         | 必填                   | 本次执行的审核模式(继承自配置,可单独覆盖)                             |
| `status`        | enum         | 必填                   | `generating` / `pending_review` / `published` / `rejected` / `failed` |
| `reviewer_id`   | string(uuid) | FK→User,可空           | manual 模式的审核人                                                   |
| `reject_reason` | text         | 可空                   | 驳回原因(rejected 时)                                                 |
| `created_at`    | datetime     | 必填                   | 调度触发时间                                                          |
| `published_at`  | datetime     | 可空                   | 实际发布时间                                                          |
...
stateDiagram-v2
    [*] --> generating: 调度触发
    generating --> published: review_mode=auto 直发
    generating --> pending_review: review_mode=manual 进队列
    generating --> failed: 生成失败
    pending_review --> published: 策展人通过
    pending_review --> rejected: 策展人驳回
    published --> [*]
    rejected --> [*]
    failed --> [*]
```

**关系**:N:1 `AiAuthorConfig`、N:1 `User`(ai_author)、N:1 `Post`、N:1 `User`(reviewer)。

> [!tip] 可追溯
> `AiPublishTask` 把 AI 作者的每次生产都记录在案——选题、模型、审核人、驳回原因全可查,呼应 weavery 的透明文化。详见 [[AI作者与内容生产#二、内容生产流程]]。

---

> [!important] 逻辑模型说明
> 以上是**逻辑模型**,字段名为**建议命名**(后端可按团队规范调整,如 `snake_case` vs `camelCase`)。物理实现细节——表拆分(如 `User.settings` 拆三张子表)、索引、缓存、读写分离、JSON 列存 vs 子表——**由后端自决**。但加粗的 **AI 相关字段**(`postOrigin` / `human_revised` / `ai_model` / `prompt_text` / `prompt_params` / `prompt_version` / `is_ai` / `ai_chips` / `version` 等)是 weavery 透明文化的数据基础,**必须保留**,不得合并或省略。`Post.version`(乐观锁)字段亦必须保留,用于多端并发编辑冲突检测(见 [[异常与边界#C2. 作者多端同时编辑]])。
...
| 方法 | 路径                 | 说明               | 鉴权 | 对应功能                          |
| ---- | -------------------- | ------------------ | ---- | --------------------------------- |
| POST | `/api/auth/register` | 注册(同意社区准则) | 无   | [[产品/功能/账户与治理#登录注册]] |
...
| 方法   | 路径                       | 说明                                 | 鉴权          | 对应功能                        |
| ------ | -------------------------- | ------------------------------------ | ------------- | ------------------------------- |
| GET    | `/api/users/:id`           | 用户主页 profile(游客可访问)         | 无            | [[产品/功能/作者主页]]          |
| PATCH  | `/api/users/me`            | 修改资料 / 设置                      | 必需 + 所有者 | [[产品/功能/账户与治理#设置]]   |
...
| DELETE | `/api/users/me`            | 注销账号(30 天冷静期,期间可取消)     | 必需 + 所有者 | [[产品/功能/账户与治理#危险区]] |

> [!info] 注销到期处理(Q24 已决:匿名化保留)
> 冷静期结束用户未恢复时,采用**匿名化保留**:清除个人信息(邮箱、密码、token、私信、关注关系、隐私设置等),但文章 / 评论保留可见,作者署名统一显示"**已注销用户**",以维持他人评论与讨论的上下文连贯。详见 [[异常与边界#E2. 注销到期永久删除]]。
...
| 方法   | 路径                      | 说明                                        | 鉴权          | 对应功能                            |
| ------ | ------------------------- | ------------------------------------------- | ------------- | ----------------------------------- |
| GET    | `/api/posts`              | 信息流 feed,`?tab=recommend/ai/follow`,分页 | 可选          | [[产品/功能/信息流]]                |
| GET    | `/api/posts/:id`          | 文章详情(含透明度卡片)                      | 可选          | [[产品/功能/帖子详情与Remix]]       |
...
| POST   | `/api/posts/:id/remix`    | Remix 改编(带 source_post_id,跳工作台)      | 必需          | [[产品/功能/帖子详情与Remix#Remix]] |

#### 草稿 Drafts

| 方法   | 路径              | 说明                                       | 鉴权          | 对应功能                        |
| ------ | ----------------- | ------------------------------------------ | ------------- | ------------------------------- |
| GET    | `/api/drafts`     | 草稿列表(含统计:全部/AI 辅助/累计字数)     | 必需 + 所有者 | [[产品/功能/创作者中心#草稿箱]] |
...
| 方法   | 路径                       | 说明                                  | 鉴权 | 对应功能                          |
| ------ | -------------------------- | ------------------------------------- | ---- | --------------------------------- |
| GET    | `/api/topics`              | 话题列表(内置 19 个)                  | 无   | [[产品/功能/话题与搜索#内置话题]] |
...
| 方法   | 路径                      | 说明                                           | 鉴权          | 对应功能                             |
| ------ | ------------------------- | ---------------------------------------------- | ------------- | ------------------------------------ |
| GET    | `/api/posts/:id/comments` | 文章评论列表(嵌套)                             | 可选          | [[产品/功能/帖子详情与Remix#评论区]] |
...
| 方法 | 路径                              | 说明                                   | 鉴权          | 对应功能                    |
| ---- | --------------------------------- | -------------------------------------- | ------------- | --------------------------- |
| GET  | `/api/conversations`              | 会话列表(头像/名/AI标签/最后消息/未读) | 必需          | [[产品/功能/社交互动#私信]] |
...
| 方法   | 路径                 | 说明                                          | 鉴权          | 对应功能                          |
| ------ | -------------------- | --------------------------------------------- | ------------- | --------------------------------- |
| GET    | `/api/favorites`     | 收藏列表,`?kind=all/post/topic/author`,分页   | 必需          | [[产品/功能/创作者中心#我的收藏]] |
...
| 方法 | 路径                            | 说明                                                                                    | 鉴权 | 对应功能                            |
| ---- | ------------------------------- | --------------------------------------------------------------------------------------- | ---- | ----------------------------------- |
| POST | `/api/ai/write`                 | 工作台 AI 调用(body: command/自由指令 + model + mode),响应含 `model` + `prompt_version` | 必需 | [[产品/功能/AI写作工作台]]          |
| GET  | `/api/ai/review/:user_id`       | AI 评价(返回当前版本,含三维 + 评语 + version)                                           | 可选 | [[产品/功能/作者主页#AI 评价]]      |
| POST | `/api/ai/review/:user_id/regen` | 重新生成评价(切下一版,`(idx+1) % 3` 循环)                                               | 可选 | AI 评价"重新生成"                   |
| GET  | `/api/ai/usage`                 | AI 用量统计,`?range=7/30/90`,返回用量数 + 来源环形图数据                                | 必需 | [[产品/功能/创作者中心#创作者数据]] |
...
> AI 作者的定期内容生产管理。原型未体现后台 UI,属管理端能力(见 [[AI作者与内容生产#六、管理后台待建]])。

| 方法  | 路径                                | 说明                                               | 鉴权        | 对应功能     |
| ----- | ----------------------------------- | -------------------------------------------------- | ----------- | ------------ |
| GET   | `/api/ai-authors`                   | AI 作者列表(含配置)                                | 必需+管理员 | 管理后台     |
| POST  | `/api/ai-authors`                   | 创建 AI 作者账号 + 配置                            | 必需+管理员 | 管理后台     |
| GET   | `/api/ai-authors/:id/config`        | 某作者的 `AiAuthorConfig`(cron/选题/模型/审核模式) | 必需+管理员 | 管理后台     |
| PATCH | `/api/ai-authors/:id/config`        | 更新配置(如切换 `review_mode`)                     | 必需+管理员 | 管理后台     |
| GET   | `/api/ai-authors/:id/tasks`         | 发布任务历史(`?status=` 筛选)                      | 必需+管理员 | 管理后台     |
| POST  | `/api/ai-authors/:id/tasks`         | 手动触发一次生产                                   | 必需+管理员 | 管理后台     |
| GET   | `/api/ai-publish/queue`             | 审校队列(`manual` 模式下待审任务)                  | 必需+策展人 | 策展人工作台 |
| POST  | `/api/ai-publish/tasks/:id/approve` | 通过审校(发布)                                     | 必需+策展人 | 策展人工作台 |
| POST  | `/api/ai-publish/tasks/:id/reject`  | 驳回(body: reason)                                 | 必需+策展人 | 策展人工作台 |

> [!important] 审核模式可配置
> `review_mode` 决定生成后是直发(`auto`)还是进审校队列(`manual`)。两种模式并存,按 `AiAuthorConfig` 配置切换,详见 [[AI作者与内容生产#四、审核模式核心]]。

#### 搜索 Search

| 方法 | 路径          | 说明                                          | 鉴权 | 对应功能                      |
| ---- | ------------- | --------------------------------------------- | ---- | ----------------------------- |
| GET  | `/api/search` | 全站搜索,`?q=关键词&type=all/post/user/topic` | 可选 | [[产品/功能/话题与搜索#搜索]] |
...
> [!note] 透明度贯穿全流程
> 以上四个数据流都涉及 AI 透明度——发布流程记录 `postOrigin`/`prompt_*`;Remix 流程传递原提示词;@AI 助手流程标记 `is_ai`;AI 评价流程返回 `version`。这是 weavery 区别于普通社区的核心数据轨迹。
