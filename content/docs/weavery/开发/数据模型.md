...
> 加粗的 AI 相关字段(`ai_generated` / `ai_model` / `prompt_text` / `prompt_params` / `prompt_version`)是 weavery "AI 内容强制标注 + 提示词公开" 制度的数据底座,必须保留并随作品持久化。社区准则第 2 条明文要求,见 [[产品/功能/账户与治理#社区准则]]。
...
| `ai_author_id`  | string(uuid) | FK→User,必填,unique | 关联的 AI 作者账号(`User.role=ai_author`,如云小织)                         |
