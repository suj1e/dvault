...
| `model`         | string       | 可空                | 生成所用默认模型(如 `GPT-4o`),见 [[待确认问题#Q6]]                         |
...
> [!important] 逻辑模型说明
> 以上是**逻辑模型**,字段名为**建议命名**(后端可按团队规范调整,如 `snake_case` vs `camelCase`)。物理实现细节——表拆分(如 `User.settings` 拆三张子表)、索引、缓存、读写分离、JSON 列存 vs 子表——**由后端自决**。但加粗的 **AI 相关字段**(`postOrigin` / `human_revised` / `ai_model` / `prompt_text` / `prompt_params` / `prompt_version` / `is_ai` / `ai_chips` / `version` 等)是 weavery 透明文化的数据基础,**必须保留**,不得合并或省略。
